...
int init_device(void)
{
    ...
    printf("width:%d\nheight:%d\npixelformat:%c%c%c%c\n", fmt.fmt.pix.width, fmt.fmt.pix.height, fmt.fmt.pix.pixelformat & 0xFF, (fmt.fmt.pix.pixelformat >> 8) & 0xFF,
...
int init_mmap()
{
    ...
    for (buf_index = 0; buf_index < req.count; buf_index++) {
        ...
        printf("Frame buffer :%d   address :0x%lx    length:%d\n", buf_index, (long)buffer[buf_index].start, buffer[buf_index].length);
    }
    return 0;
}
